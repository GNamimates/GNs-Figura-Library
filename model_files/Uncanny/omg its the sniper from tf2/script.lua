...
function tick()
    ...
    if player.isOnGround() ~= lastgrounded then--triggers once is grounded or not
    ...
    if player.isOnGround() or player.isWet() then
            ...
            if player.isSneaky() then
                ...
                if veldist > 0.01 then
                    if isSprinting and dotp(localVel.x) > 0 then
                        ...
                        pose.head = {20,0,0}
                        pose.legLeft = {(math.sin(distWalked)*50+20)*moveMult,0,-1}
                        pose.footleft = {(math.sin(distWalked+math.rad(-(90*dotp(localVel.x+0.01))))*(45)-45)*moveMult,0,0}
                        pose.legRight = {(math.sin(distWalked)*-50+20)*moveMult,0,1}
                        pose.footRight = {(math.sin(distWalked+math.rad(-(90*dotp(localVel.x+0.01))))*-45-45)*moveMult,0,0}
                        pose.body = {localVel.x*-20-20,math.sin(distWalked)*5,localVel.z*60}
                        model.Body.setPos({0,-math.abs(math.sin(distWalked)),0})
    ...
    if client.isHost() then
        network.ping("punched",attackKey.isPressed() or interactKey.isPressed())
    end
    if isPunching then
        if lastPunch > 3 then
                --clicked
...
function render(delta)
    ...
    model.Body.MIMIC_HEAD.offset.setRot(tableLerp(model.Body.MIMIC_HEAD.offset.getRot(),pose.head,delta))
    model.Body.setRot(tableLerp(model.Body.getRot(),pose.body,delta))
