...
for i=1,instanceCount do
    local base = model.NO_PARENT["AGENT" .. i]
    agents[i] = {
    base = base,
    B = base["B"..i],
    H = base["H"..i],
    ...
    if finishedLoading then
        for key, value in pairs(agents) do
            ...
            if wanderTimer[key] < 0 then
                ...
                offsetTarget[key] = vectors.of({math.random()*20-10,0,math.random()*20-10})
            end
...
function world_render(delta)
    if finishedLoading then
        for key, value in pairs(agents) do
            value.base.setPos(vectors.lerp(value.base.getPos(),vectors.of({position[key].x*-16,position[key].y*-16,position[key].z*16}),0.5 ))

            if velocity[key].distanceTo(vectors.of({})) > 0.01 then
                value.base.setRot({0,lerp_angle(value.B.getRot().y,math.deg(math.atan2(velocity[key].x,velocity[key].z))+180,0.9)})
            end
            ...
            value.LL.setRot({lerp(value.LL.getRot().x,swing,1)})
            value.RL.setRot({lerp(value.RL.getRot().x,-swing,1)})
            value.LA.setRot({lerp(value.LA.getRot().x,swing,1)})
            value.RA.setRot({lerp(value.RA.getRot().x,-swing,1)})
        end
    end
end
