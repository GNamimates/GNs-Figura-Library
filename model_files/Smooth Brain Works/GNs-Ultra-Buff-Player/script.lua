...
--========================================================--
--GNs animated player by GNamimates (HEAVILY MODIFIED FOR BUFF STEVE :P)
--version 1.1
...
function tick()
    ...
    shake(shakeIntensity.x+veldist*0.05,shakeIntensity.y+veldist*0.05)
    if player.isGrounded() ~= lastgrounded then--triggers once is grounded or not
        ...
        shake(0.1,0.2)
        sound.playSound('entity.lightning_bolt.thunder',{player.getPos().x,player.getPos().y,player.getPos().z,0.5,0.8})
        sound.playSound('entity.generic.explode',{player.getPos().x,player.getPos().y,player.getPos().z,0.5,0.5})
    end
    ...
    if not player.isWet() then
        if player.isGrounded() then
            ...
            if isClimbing then
                ...
                pose.armLeft = {-math.sin(altitudeClimbed*PI*1)*90+90,-math.cos(altitudeClimbed*PI*2)*22+22,0}
                pose.armRight = {-math.sin(altitudeClimbed*PI*1)*-90+90,-math.cos(altitudeClimbed*PI*2)*22-22,0}
                pose.handLeft = {-math.sin(altitudeClimbed*PI*1)*-40+40,0,0}
                pose.handRight = {-math.sin(altitudeClimbed*PI*1)*40+40,0,0}
            else
        ...
        if isSprinting then
            ...
            lastPunch = 100
            pose.head = {0,0,0}
            pose.body = {0,math.sin(distWalked*0.5)*10+math.sin(distWalked*2)*2.5,0}
            ...
            pose.armLeft = {math.cos(distWalked*0.5)*40,0,math.sin(distWalked*1)*40-40}
            pose.armRight = {-math.cos(distWalked*0.5)*40,0,-math.sin(distWalked*1)*-40+40}
            pose.handLeft = {0,0,math.sin(distWalked*0.5)*40-40}
            pose.handRight = {0,0,math.sin(distWalked*-0.5)*-40+40}
    ...
    if lastPunch < 20 then--cooling down arm
        pose.armRight[2] = 90 + pose.armRight[2]
        pose.armLeft[2] = -90 + pose.armLeft[2]
        pose.armRight[1] = -90 + pose.armRight[1]
        pose.armLeft[1] = -90 + pose.armLeft[1]
        pose.handLeft[1] = 90
        pose.handRight[1] = 90
    end
...
function render(delta)
    camera.THIRD_PERSON.setPos(vectors.of{(math.random()*2-1)*shakeIntensity.x,(math.random()*2-1)*shakeIntensity.y,0})
    camera.FIRST_PERSON.setPos(vectors.of{(math.random()*2-1)*shakeIntensity.x,(math.random()*2-1)*shakeIntensity.y,0})
    shakeIntensity.x = shakeIntensity.x*shakeDamping
    shakeIntensity.y = shakeIntensity.y*shakeDamping
